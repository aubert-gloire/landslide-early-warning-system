...
const QA = [
  ...
  {
    triggers: ["how does it work", "how does the system work", "explain the system", "overview", "what does this system do", "how does ews work", "walk me through", "explain to me", "new here", "first time", "getting started", "what is this", "how does this work", "end to end", "full picture"],
    answer: `Here is how the system works from start to finish:\n\n━━ STEP 1 — RAINFALL COLLECTION (every morning)\nNASA satellites measure how much rain fell across Northern Province the previous day. That data arrives at about 08:00 AM Kigali time via GPM IMERG. If it's not ready yet, the system automatically falls back to CHIRPS satellite data.\n\n━━ STEP 2 — TERRAIN + SOIL CONTEXT\nNorthern Province is divided into 250 slope units — sections of hillside that each drain water in one direction. For every unit, the system already knows: how steep it is, how water pools there, what the soil type is, and how much vegetation covers it. These are permanent facts stored in the database.\n\n━━ STEP 3 — RISK SCORING\nRainfall from today + terrain data from the database = a feature set for each slope unit. An XGBoost machine learning model (trained on 14 years of historical data) processes all 250 units and gives each one a risk score from 0–100%.\n\nThe most important factor: how much rain has fallen in the PAST 5 DAYS — not just today. Saturated soil from consecutive days of rain is what makes slopes dangerous.\n\n━━ STEP 4 — ALERT DECISION\nIf any slope unit scores 5% or above, an SMS alert is sent immediately to the registered officer for that district. The threshold is intentionally low — it is better to send a precautionary alert than to miss a real event. If an earthquake was detected nearby in the last 48 hours, the threshold drops further to 3%.\n\n━━ STEP 5 — SMS DISPATCH\nThe alert goes out via Telerivet, routed through a real Android SIM on MTN Rwanda. The SMS tells the officer: which sector is at risk, the risk level, the GPS location, and the main driver.\n\n━━ STEP 6 — OFFICER RESPONSE\nThe officer goes to the location and replies by SMS — YES to confirm the risk, NO if conditions on the ground look safe. That reply is logged in the Alerts tab and tracked as the system's real-world accuracy.\n\n━━ WHAT YOU SEE ON THIS DASHBOARD\n• Risk Map — 250 coloured polygons showing today's scores\n• Districts tab — peak risk per district with slope unit counts\n• Alerts tab — full SMS log with officer feedback\n• Predict tab — test any scenario manually\n• Run Pipeline — trigger a fresh assessment right now`,
  },
  {
    triggers: ["risk map", "map", "colour", "color", "polygon", "what do the colours", "what do colors mean"],
    answer: `The Risk Map shows all 250 slope units in Northern Province as coloured polygons:\n\nGreen — low risk (below 50%)\nAmber — moderate risk (50–79%)\nRed — high risk (80%+) — SMS alert dispatched\n\nClick any polygon to see the unit ID, district, sector, risk %, and whether an alert was sent.\n\nThe map legend shows:\n• Assessment date — when the pipeline last ran\n• Rainfall data to — the date of the most recent rainfall data used\n\nNote: the map always shows the most recent pipeline results. Run the pipeline to refresh it with today's data.`,
  },
  {
    triggers: ["pipeline", "run pipeline", "what is pipeline", "trigger", "cron", "schedule", "daily", "what does pipeline do"],
    answer: `The pipeline is the core process that runs every morning. Click "Run Pipeline" in the top-right to trigger it manually.\n\nWhat it does step by step:\n1. Downloads yesterday's rainfall from NASA GPM IMERG (~14h lag). Falls back to CHIRPS if IMERG isn't ready.\n2. Checks USGS earthquake data — if a M4.0+ earthquake occurred within 200km in the last 48 hours, the alert threshold is lowered automatically.\n3. Builds a feature matrix combining rainfall + terrain data for all 250 slope units.\n4. Runs XGBoost inference — scores every slope unit.\n5. Saves all predictions to the database.\n6. Sends SMS alerts to field officers for any unit above the threshold.\n\nIt also runs automatically at 01:00 AM Kigali time daily via GitHub Actions.`,
  },
  {
    triggers: ["alert", "sms", "send alert", "dispatch", "officer", "expert", "when are alerts sent", "who gets the sms"],
    answer: `SMS alerts are sent automatically during the pipeline when any slope unit exceeds the risk threshold.\n\nEach SMS includes:\n• District and sector of the high-risk slope unit\n• Risk level (WATCH / WARNING / EMERGENCY)\n• Risk percentage\n• GPS coordinates of the highest-risk point\n• The main driver (e.g. "antecedent 5day mm")\n• Instructions: reply YES [unit_id] to confirm or NO [unit_id] to dispute\n\nAlerts go through Telerivet (Android SIM route) — chosen after Africa's Talking's shared sender-ID pool proved unreliable on MTN Rwanda.\n\nYou can also send a manual alert from the Predict panel using real field measurements.`,
  },
  {
